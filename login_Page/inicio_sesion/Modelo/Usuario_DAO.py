...
class UsuarioDTO:
    ...
    def __init__(self):
        ...
        self._SQL_DELETE_USER = "DELETE FROM users WHERE email = %s"
        self._SQL_UPDATE_USER = "UPDATE users SET email = %s, password = %s WHERE id = %s"
        self._SQL_VALIDATE_USER = "CALL inicio_sesion(%s,%s)"
        self._SQL_SEARCH_USER = "CALL Search_user(%s,%s)"
        self._SQL_SELECT_USER = "SELECT Nombre, Correo, Contrasena FROM usuarios WHERE codigo = %s"
    ...
    def update_user(self, usuario: Usuario):
        return self.db.execute_query(self._SQL_UPDATE_USER,(usuario.get_email(),usuario.get_password())) # type: ignore
    
    def delete_user(self, usuario: Usuario):
        return self.db.execute_query(self._SQL_DELETE_USER,(usuario.get_email())) # type: ignore
